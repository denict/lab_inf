...
def yaml_to_json_regex(input_string: str):
    """конвертирование из YAML в JSON"""
    # data = [string + "\n" for string in input_string.split("\n") if
    #         string.replace(" ", "") != "" and string.replace(" ", "") != "#"]
    data = [string + "\n" for string in input_string.split("\n") if
            re.match(r'^\s*$', string) is None and re.match("r'\s*#[\w\s]*$", string) is None] # re.match - соответствие, начиная с начала строки
    ...
    for i in range(len(data)):
        ...
        # if not (stri.endswith(":\n") or stri.endswith(": \n")):  # строка не оканчивается на ":"
        if not re.search(": ?\n$", stri): # строка не оканчивается на ":"
    ...
    for i in range(1, len(res_str) - 1):  # кол-во отступов увеличиваем на 2
        ...
        if re.search(r"]", res_str[i]) and not re.search(r'}', res_str[i]) and not re.search(r'}',res_str[i+1]) and i < len(res_str) - 2:
            res_str[i] = res_str[i][:res_str[i].index("]")] + "]," + res_str[i][res_str[i].index("]") + 1:]  # добавление запятой после ] в одном случае

        # if "]" not in res_str[i] and "[" not in res_str[i] and "{" not in res_str[i] and "}" not in res_str[i] and "'" not in res_str[i]: # добавление скобкам в значение
        # if not re.search(r"\]|\}|\{|\[|\'", res_str[i]):
...
#         print(yaml_to_json_regex(yaml_string))